...
export function CartPage() {
  ...
  return <section className="bg-[#f5f9f7] py-8 text-[#101715] md:py-12"><div className="mx-auto max-w-[1400px] px-5 md:px-8">
    ...
    {!items.length ? <div className="flex min-h-80 flex-col items-center justify-center border border-dashed border-black/20 bg-white px-6 text-center"><ShoppingBag className="size-10 text-primary" /><h2 className="mt-4 font-display text-2xl font-bold uppercase">Giỏ hàng đang trống</h2><p className="mt-2 max-w-sm text-sm text-neutral-600">Khám phá grooming và merchandise được tuyển chọn tại TOTO.</p><Button asChild className="mt-6"><Link href="/shop">Tiếp tục mua sắm</Link></Button></div> : <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="h-fit border border-black/10 bg-white p-5 lg:sticky lg:top-24"><h2 className="font-display text-2xl font-bold uppercase">Tóm tắt đơn hàng</h2><div className="mt-5 border-y border-black/10 py-4"><label className="text-xs font-bold uppercase tracking-wide">Mã giảm giá</label><div className="mt-2 flex gap-2"><input value={couponInput} onChange={(event) => setCouponInput(event.target.value)} placeholder="Nhập mã giảm giá" className="min-w-0 flex-1 border border-black/20 px-3 text-sm outline-none focus:border-primary" /><Button type="button" onClick={applyCoupon} disabled={isApplying}>{isApplying ? "Đang xử lý..." : "Áp dụng"}</Button></div>{couponError ? <p className="mt-2 text-xs text-destructive">{couponError}</p> : null}{coupon ? <p className="mt-2 text-xs text-primary">Đã áp dụng mã {coupon}</p> : null}</div><dl className="space-y-3 py-5 text-sm"><div className="flex justify-between"><dt>Tạm tính</dt><dd>{formatCurrency(subtotal)}</dd></div><div className="flex justify-between"><dt>Phí vận chuyển</dt><dd><span className="text-primary text-[11px] italic">Tính lúc thanh toán</span></dd></div>{discount ? <div className="flex justify-between text-primary"><dt>Giảm giá</dt><dd>-{formatCurrency(discount)}</dd></div> : null}</dl><div className="flex items-end justify-between border-t border-black/10 pt-4"><span className="font-semibold">Thành tiền</span><strong className="font-display text-2xl text-[#d71920]">{formatCurrency(total)}</strong></div><Button asChild className="mt-5 h-12 w-full bg-[#101715] uppercase hover:bg-[#101715]/80"><Link href="/checkout">Tiến hành thanh toán →</Link></Button><div className="mt-4 flex gap-2 text-xs text-neutral-500"><Truck className="mt-0.5 size-4 text-primary" />Miễn phí vận chuyển khi thanh toán trước qua PayOS.</div></aside>
    </div>}
