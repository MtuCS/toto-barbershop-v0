...
export function CheckoutForm() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      setForm((current) => ({ ...current, name: user.name || current.name, phone: user.phone || current.phone, email: user.email || current.email, address: address ? `${address.street}, ${address.ward}, ${address.district}, ${address.province}` : current.address }))
    }, 0)
