...
export function HomeValues() {
  return (
    <section
      ...
      className="relative isolate overflow-hidden bg-[#07110f] px-5 py-16 text-[#f2f5f3] md:px-8 md:py-24 lg:px-10 xl:px-14"
    >
      <div
        aria-hidden="true"
        ...
        <div className="mt-10 grid gap-6 md:mt-14 md:grid-cols-3">
          {values.map((value) => {
            ...
            return (
              <article
                key={value.title}
                className="group relative flex min-h-[290px] flex-col justify-between overflow-hidden rounded-xl border border-white/12 bg-[#0d1f1c]/80 p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[#2f7a68]/60 hover:bg-[#0f2420] hover:shadow-[0_16px_40px_rgba(7,17,15,0.6)] md:p-8"
              >
                <div className="relative z-10">
                  ...
                  <h3 className="mt-6 text-2xl font-bold tracking-tight text-white md:text-3xl">
                    {value.title}
                  </h3>
                  <p className="mt-4 max-w-[34ch] text-sm leading-relaxed text-white/70 transition-colors duration-300 group-hover:text-white/85">
                    {value.description}
                  </p>
                </div>
