...
export function LookbookSection({ items }: { items: LookbookItem[] }) {
  const shopItems = items.filter((item) => item.category === "Shop")
  const hairItems = items.filter((item) => item.category !== "Shop")
...
function SectionHeader({ label, title, copy }: { label: string; title: string; copy: string }) {
  return (
    <div className="max-w-4xl">
      {label ? <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#79b8a7]">{label}</p> : null}
      <h2 className="mt-4 font-display text-4xl font-bold uppercase leading-tight tracking-tight text-[#f2f5f3] md:text-6xl lg:text-7xl">{title}</h2>
